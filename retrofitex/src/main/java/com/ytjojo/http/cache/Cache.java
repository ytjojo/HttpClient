...
public final class Cache implements Closeable, Flushable {
   ...
   */
  public List<String> allKeys1() {
    List<String> nameFiles = new ArrayList<>();
    File cacheDirectory = cache.getDirectory();
  ...
  public List<String> allKeys() {
    ...
    try {
      ...
      while (snapshots.hasNext()){
        nameFiles.add(snapshots.next().key());
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  Response get(Request request,String dynamicKey,String dynamicKeyGroup) {
    String key = composeKey(request,dynamicKey,dynamicKeyGroup);
  ...
  CacheRequest put(Response response , String dynamicKey,String dynamicKeyGroup) {
    String requestMethod = response.request().method();

    if (invalidatesCache(response.request().method())) {
      try {
        remove(response.request(),dynamicKey,dynamicKeyGroup);
      } catch (IOException ignored) {
        // The cache cannot be written.
      }
      return null;
    ...
    DiskLruCache.Editor editor = null;
    try {
      editor = cache.edit( composeKey(response.request(),dynamicKey,dynamicKeyGroup));
      if (editor == null) {
        return null;
      }
      entry.writeTo(editor);
  ...
  private static final class Entry {
    ...
    private final String requestBodyContent;
    ...
    public Entry(Source in) throws IOException {
      try {
        ...
        if(requestMethod.equals("POST")){
          requestBodyContent = source.readUtf8LineStrict();
          contentType = source.readUtf8LineStrict();
        }else{
    ...
    public Entry(Response response) {
      ...
      this.requestBodyContent = buffer.readUtf8();
      this.contentType = response.request().body().contentType().type()+"/"+ response.request().body().contentType().subtype()+"; charset="+
          response.request().body().contentType().charset().name();
    }
    ...
    public void writeTo(DiskLruCache.Editor editor) throws IOException {
      ...
      sink.writeUtf8(RECEIVED_MILLIS)
          ...
          .writeByte('\n');
      if(requestMethod.equals("POST")){
        sink.writeUtf8(requestBodyContent);
        sink.writeByte('\n');
    ...
    public Response response(DiskLruCache.Snapshot snapshot) {
      ...
      if(requestMethod.equals("POST")){
        requestBody = RequestBody.create(MediaType.parse(contentType),requestBodyContent);
      }
  ...
  public static Headers.Builder addLenient(Headers.Builder builder,String line){
    ...
    if (index != -1) {
      builder.add(line.substring(0, index), line.substring(index + 1));
      return builder;

    } else if (line.startsWith(":")) {
      // Work around empty header names and header names that start with a
      // colon (created by old broken SPDY versions of the response cache).
      builder.add("", line.substring(1)); // Empty header name.
      return builder;
    } else {
      builder.add("", line); // No header name.
      return builder;
    }
  }
